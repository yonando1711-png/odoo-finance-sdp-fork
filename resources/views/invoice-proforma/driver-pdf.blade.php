            ...
            <table class="lines-table">
                ...
                <tbody>
                    @php
                        ...
                        $cleanProformaDesc = function($line) {
                            $desc = $line->clean_description ?? $line->description ?? '';
                            if (!empty($line->product_name)) {
                                $escaped = preg_quote($line->product_name, '/');
                                $desc = preg_replace('/^' . $escaped . '[\r\n\s]*/i', '', $desc);
                            }
                            $desc = preg_replace('/^Lain-Lain\s*\(inv\)[\r\n\s]*/i', '', $desc);
                            return trim($desc);
                        };
                        ...
                        if (isset($printMode) && $printMode === 'summary') {
                            ...
                            if ($noteLines->isNotEmpty()) {
                                ...
                                $summaryDesc = $regularLines->map(fn($d) => $cleanProformaDesc($d))->filter()->implode("\n");
                            }
                            ...
                            <td class="text-center">
                                @if(!isset($printMode) || $printMode !== 'summary')
                                    ...
                                    @if($displayQty > 0 && !request('without_satuan'))
                                        {{ ($displayQty == (int)$displayQty) ? number_format($displayQty, 0, '.', ',') : number_format($displayQty, 2, '.', ',') }} Org.
                                    @endif
                                @endif
                            </td>
                            <td class="text-right">
                            ...
                            @php
                                ...
                                if (!$isSummary && isset($noteLines) && $noteLines->isNotEmpty()) {
                                    foreach($noteLines as $noteL) {
                                        $noteText = trim($cleanProformaDesc($noteL));
